...
public final class MsgEchoServer {
	static final int PORT = Integer.parseInt(System.getProperty("port", "10005"));
	private static final Logger myLog = Logger.getLogger(MsgEchoServer.class);
	public static void main(String[] args) throws Exception {
		...
		try {
			myLog.info("PORT:" + PORT);
			myLog.info("System.getProperty:" + System.getProperty("port", "10005"));
			...
			//				}
			//			});
			boot.group(acceptGroup, connectGroup).channelFactory(NioUdtProvider.MESSAGE_ACCEPTOR).option(ChannelOption.SO_BROADCAST, true).handler(new ChannelInitializer<UdtChannel>() {
				@Override
				public void initChannel(final UdtChannel ch) throws Exception {
					ch.pipeline().addLast(new MsgEchoServerHandler());
				}
